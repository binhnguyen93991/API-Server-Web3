...
async function getName(tokenAddres) {
  let tokenRouter = await new web3.eth.Contract(tokenAbi, tokenAddres);
  let name = await tokenRouter.methods.name().call();
  return name;
}

async function getSymbol(tokenAddres) {
  let tokenRouter = await new web3.eth.Contract(tokenAbi, tokenAddres);
  let symbol = await tokenRouter.methods.symbol().call();
  return symbol;
}

async function getDivisor(tokenAddres) {
  let tokenRouter = await new web3.eth.Contract(tokenAbi, tokenAddres);
  let divisor = await tokenRouter.methods.decimals().call();
  return divisor;
}

async function circulatingSupply(tokenAddres){
  let tokenRouter = await new web3.eth.Contract(tokenAbi, tokenAddres);
  let tokenDecimals = await tokenRouter.methods.decimals().call();
...
async function totalSupply(tokenAddres) {
  let tokenRouter = await new web3.eth.Contract(tokenAbi, tokenAddres);
  let tokenDecimals = await tokenRouter.methods.decimals().call();
  let totalSupply = await tokenRouter.methods.totalSupply().call();

  return parseInt(totalSupply)/10 **parseInt(tokenDecimals);
}
/*
How it works?
...
app.get('/:contractAddress', authenToken, asyncHandler(async (req, res, next) => {
  ...
  let circulating = await circulatingSupply(tokenAddres);
  let name = await getName(tokenAddres);
  let divisor = await getDivisor(tokenAddres);
  let symbol = await getSymbol(tokenAddres);
